/**
 * Represents a single scoring rule for chart recommendation.
 * Each rule evaluates a specific aspect of the data and contributes to the overall score of a chart type.
 */
export interface Rule {
  ...
  name: string;
  /**
   * The weight of the rule's score in the final calculation.
   */
  weight: number;
  /**
   * The function that performs the check.
   * @param data The chart data to be evaluated.
  ...
  check: (
    ...
    /**
     * Whether the data passes the rule's check.
     */
    passed: boolean;
    /**
     * The score assigned by the rule, typically between 0 and 1.
     */
    score: number;
    /**
     * Optional details or reasons for the score.
     */
    details?: string;
  };
}

export interface ScorerFn {
  (data: ChartData, config: ScoringConfig): ScoreResult;
}

/**
 * Represents the input data structure for chart recommendation.
 * This is a generic representation of 2D data.
 */
export interface ChartData {
  /**
   * The dataset for chart generation, represented as an array of objects.
   * @example [{ product: 'A', sales: 100 }, { product: 'B', sales: 150 }]
   */
  data: Record<string, any>[];
  /**
   * An array of field names to be treated as dimensions.
   */
  dimensions: string[];
  /**
   * An array of field names to be treated as metrics.
   */
  metrics: string[];
  /**
   * The target chart type for evaluation.
   */
  chartType: string;
}
...
export interface ScoreResult {
  ...
  /**
   * An array of results from each individual rule check.
   * Useful for debugging or providing detailed feedback.
   */
  ruleResults?: { passed: boolean; score: number; details?: string }[];
  /**
   * The chart type for which the score was calculated.
   */
  chartType?: string;
}

// Old types, with necessary renames to avoid conflicts
...
export interface OldScoreResult {
  chartType: ChartType;
  originScore: number;
  fullMark: number;
  score: number;
...
export type OldScorer = (params: ScorerParams) => Array<() => OldScoreResult>;
